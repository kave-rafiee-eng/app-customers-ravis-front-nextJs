...
export default function PhoneBook() {
  ...
  const [newDescriptionPersian, setNewDescriptionPersian] = React.useState("");
  ...
  async function loadData() {
    try {
      const res = await api.get("/phonebook");
  ...
  const translator = async (persian: string): Promise<DescriptionType> => {
    const result = await apiTranslate.post("/translate", { text: persian });
  ...
  const handleModalTranslate = async () => {
    ...
    if (persian.length < 2) {
      addMessage("Persian description is required for translation", "error");
      return;
    }
  ...
  const handleCreate = async () => {
    ...
    setSaving(true);
    try {
      await api.post("/phonebook", payload);
  ...
  const handleSaveAll = async () => {
    ...
    try {
      for (const item of listPhone) {
        await api.patch(`/phonebook/${item.id}`, {
          name: item.name,
          phone: item.phone,
          ...
          return (
            <TextField
              ...
              onChange={(event) => {
                ...
                setListPhone((prev) =>
                  prev.map((item) =>
                    item.id === row.id ? { ...item, phone: value } : item,
                  ),
                );
              }}
            />
        ...
        label: () => {
          return (
            <FormControl size="small" sx={{ minWidth: 120 }}>
              <InputLabel id="phonebook-lang-label">lang</InputLabel>
              <Select
        ...
        <Box sx={styleModal} display="flex" flexDirection="column" gap={2}>
          ...
          <Stack spacing={2}>
            ...
            <TextField
              ...
              onChange={(event) => setNewDescriptionPersian(event.target.value)}
              dir="rtl"
            />
          </Stack>

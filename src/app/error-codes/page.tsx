...
import { errorCodeForAi } from "./ErrorCodeForAi";
...
export default function ErrorCodeHome() {
  ...
  const handleDownloadForAi = () => {
    ...
    const json = JSON.stringify(errorCodeForAi(allErrors), null, 2);
    const blob = new Blob([json], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    // link.download = `error-codes-${new Date().toISOString().slice(0, 10)}.json`;
    link.download = `errorCodesAi.json`;
    ...
    >
      ...
      <Box sx={{ background: "#1B3C53", height: "10%" }}>
        ...
        >
          ...
          <Button variant="contained" onClick={handleDownloadForAi}>
            download For Ai
          </Button>
        </Stack>
      </Box>

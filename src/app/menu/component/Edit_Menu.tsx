...
import { API_BACKEND } from "@/app/constant";
...
export default function EditMenu({
    ...
    useEffect(() => {
        async function getMenu(id: string) {
            try {
                const resault = await API_BACKEND.get(
    ...
    const handleDeleteGroupItem = () => {
        ...
        const nextGroup = group.filter(
            (_, index) => index !== activeIndexGroup,
        );
    ...
    if (checkTypeMenu(menuState) == typeMenuEnum.SETTING_MULTY_GROUP) {
        if (
            menuState?.data.settingMultyGroup![activeIndexGroup]
                .settingOneParameter
        )
    ...
    return (
        ...
        >
                ...
                >
                    {checkTypeMenu(menuState) ==
                        ...
                        <FormControl
                            ...
                            sx={{ minWidth: 140 }}
                        >
                            <InputLabel id="edit-menu-items-label">
                                Items
                            </InputLabel>
                            <Select
                            ...
                            >
                                {menuState!.data.settingMultyGroup!.map(
                                    (settingMultyGroup, index) => {
                                        let name = "";
                                        if (
                                            settingMultyGroup.settingOneParameter
                                        )
                                            name =
                                                settingMultyGroup
                                                    .settingOneParameter.label;
                                        if (settingMultyGroup.settingOneSelect)
                                            name =
                                                settingMultyGroup
                                                    .settingOneSelect.label;
                                        return (
            ...
            >
                ...
                <DialogContent>
                    <DialogContentText>
                        ...
                        <strong>
                            {menuState?.lable ?? menuState?.id ?? idEdit}
                        </strong>{" "}
                        (id: {menuState?.id ?? idEdit})? this action cannot be
                        undone.
                    </DialogContentText>
                </DialogContent>
                <DialogActions>
            ...
            {checkTypeMenu(menuState) == typeMenuEnum.SUBMENU && (
                <EditSubMenu />
            )}
